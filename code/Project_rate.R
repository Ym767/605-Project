...
dev.off()
rate = (knn1$prediction[40] - knn1$prediction[20])/knn1$prediction[20]
df <- data.frame(name = dataname,
                 rate = rate
)
names(df) <- c("name", "rate")
write.csv(df,paste(dataname,"csv",sep = "."), row.names = FALSE)
